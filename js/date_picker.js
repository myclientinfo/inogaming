...
function updateDateField(dateFieldName, dateString)
{
	var targetDateField = $(dateFieldName);
	if (dateString){
		var split_date = targetDateField.value.split(' ');
        if(split_date.length == 2){
            targetDateField.value = dateString + ' ' + split_date[1];
        } else {
            targetDateField.value = dateString + ' 00:00:00';
        }
        var long_date_string = makeLongDateString(dateString);
		if(!long_date_string){
			$(dateFieldName+'_display').innerHTML = 'Click to set date &raquo;';
		} else {
			$(dateFieldName+'_display').innerHTML = long_date_string;
		}
	}
	var pickerDiv = document.getElementById(datePickerDivID);
...
function makeShortDateString(date_string){
    ...
    return display_date.getDate()+'/'+(display_date.getMonth()+1)+'/'+display_date.getFullYear();
}
